...
namespace GameScene
{
    ...
    {
        public static bool CheckRotationAvailabilityWitDisplacements(Transform tetrominoTransform, Tetromino tetrominoStats, TetrisGrid grid, Vector3 gridPosition)
        {
            ...
            foreach (Transform childTile in tetrominoTransform)
            {
                ...
                if (grid.CheckIfFieldEmpty(y, x) == false)
                {
                    ...
                    foreach (Transform childTile2 in tetrominoTransform)
                    {
                        ...
                        int privY = (int)gridPosition.y + grid.GetMaxAvailableHeight(grid.height * 2, x);
                        if (privY > minY)
                        {
                            minY = privY;
                        }
